...
export function DesignCanvas() {
  ...
  const editDesignId = urlParams.get('edit');
  ...
  // Load existing design if edit parameter is present
  useEffect(() => {
    if (editDesignId && user && products.length > 0) {
      loadExistingDesign(editDesignId);
    }
  }, [editDesignId, user, products]);
  ...
  const fetchProducts = async () => {
    try {
      ...
      setProducts(data);
      // Set the first product as default if available (only if not editing)
      if (data.length > 0 && !editDesignId) {
  ...
  const loadExistingDesign = async (designId: string) => {
    ...
    try {
      ...
      const product = products.find(p => p.id === design.designData.product.id);
      if (product) {
      ...
      setSelectedColor(design.designData.color || 'white');
      setSelectedSize(design.designData.size || 'M');
