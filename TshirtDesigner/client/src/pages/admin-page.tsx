...
import { insertProductSchema, type Product, type InsertProduct, type Order, type User } from "@shared/schema";
...
export default function AdminPage() {
  ...
  const { data: customerOrders = [], isLoading: customerOrdersLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/admin/customers/${selectedCustomerId}/orders`);
      if (!response.ok) {
  ...
  const onUpdateProduct = async (data: InsertProduct) => {
          ...
          <Dialog open={isEditProductOpen} onOpenChange={(open) => {
            ...
            <DialogContent className="max-w-2xl">
              ...
              <Form {...editProductForm}>
                <form onSubmit={editProductForm.handleSubmit(onUpdateProduct)} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <FormField
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Description</FormLabel>
                        <FormControl>
                          <Textarea {...field} data-testid="textarea-edit-product-description" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Base Price</FormLabel>
                          <FormControl>
                            <Input type="number" step="0.01" {...field} data-testid="input-edit-product-price" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>Image URL</FormLabel>
                          <FormControl>
                            <Input {...field} data-testid="input-edit-product-image-url" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                  ...
                  <div className="flex justify-end space-x-4">
                    ...
                    <Button type="submit" disabled={editProductForm.formState.isSubmitting} data-testid="button-update-product">
                      {editProductForm.formState.isSubmitting ? "Updating..." : "Update Product"}
                    </Button>
                  </div>
                </form>
              </Form>
                  ...
                  <CardContent className="p-6">
                    ...
                    {customersLoading ? (
                      ...
                      <div className="text-center py-12">
                        ...
                        <p className="text-muted-foreground">Customers will appear here once they start registering.</p>
                      </div>
                    ) : (
                      <div className="grid lg:grid-cols-2 gap-6">
                        {/* Customer List */}
                        <div className="space-y-4">
                          <h4 className="font-semibold text-foreground">Customer List</h4>
                          <div className="space-y-2 max-h-96 overflow-y-auto">
                            {customers
                              .filter((customer: User & { orderCount: number; totalSpent: number }) =>
                                customer.username.toLowerCase().includes(customerSearchQuery.toLowerCase()) ||
                                customer.email.toLowerCase().includes(customerSearchQuery.toLowerCase()) ||
                                (customer.firstName && customer.firstName.toLowerCase().includes(customerSearchQuery.toLowerCase())) ||
                                (customer.lastName && customer.lastName.toLowerCase().includes(customerSearchQuery.toLowerCase()))
                              )
                              .map((customer: User & { orderCount: number; totalSpent: number }) => (
                                <Card 
                                  key={customer.id} 
                                  ...
                                  <CardContent className="p-4">
                                    <div className="flex items-center justify-between">
                                      ...
                                      <div className="text-right">
                                        ...
                                        <p className="text-sm text-muted-foreground">
                                          ${Number(customer.totalSpent || 0).toFixed(2)} total
                                        </p>
                                      </div>
                                    </div>
                                  </CardContent>
                        ...
                        {/* Customer Orders */}
                        <div className="space-y-4">
                          <h4 className="font-semibold text-foreground">Customer Orders</h4>
                          {selectedCustomerId ? (
                            customerOrdersLoading ? (
                              ...
                              <div className="text-center py-8">
                                ...
                                <p className="text-muted-foreground">No orders from this customer</p>
                              </div>
                            ) : (
                              <div className="space-y-2 max-h-96 overflow-y-auto">
                            ...
                            <div className="text-center py-8">
                              ...
                              <p className="text-muted-foreground">Select a customer to view their orders</p>
                            </div>
                          )}
                        </div>
                      </div>
